...
import { web3formsConfig, personalInfo, socialLinks } from '../data/portfolioData';
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!permission) {
      setStatus('error');
      setFeedbackMessage('Please agree to the contact permission.');
      setTimeout(() => {
        setStatus('idle');
        setFeedbackMessage('');
      }, 4000);
    ...
    const accessKey = web3formsConfig.accessKey;

    if (!accessKey) {
      setStatus('error');
      setFeedbackMessage("Something went wrong. Please try again or email me directly.");
      setTimeout(() => {
        setStatus('idle');
        setFeedbackMessage('');
      }, 5000);
    ...
    try {
      const response = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          to_email: personalInfo.emails.primary,
        }),
      });
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-12 md:gap-16 w-full">
            ...
            <div className="flex flex-col md:flex-row gap-12 mt-4">
              ...
              <div className="flex-1 flex flex-col gap-6 text-xs text-white/70 font-medium">
                ...
                {feedbackMessage && (
                  <p className={`text-xs font-bold ${status === 'success' ? 'text-green-200' : 'text-yellow-200'}`}>
                    {feedbackMessage}
                  </p>
                )}
